...
const (
	...
	attrBlink
	attrWrap
	attrFaint // SGR 2 (faint/dim) — e.g. Ink "dimColor" ghost text
)
...
type State struct {
	...
	// scrollback retains lines that scroll off the top of the (non-alternate)
	// screen so a host can offer scrollback for inline programs. Each entry is
	// a copy of an evicted line, trailing blanks trimmed. Disabled while
	// scrollbackMax == 0. Capture happens inside scrollUp under t.mu.
	scrollback    []line
	scrollbackMax int
	// scrollbackEvicted is the monotonic count of lines ever captured into
	// scrollback. It keeps growing even as the ring drops old lines, so a
	// consumer can compute how many new lines appeared since it last looked
	// (the ring length alone can't, once it saturates at scrollbackMax).
	scrollbackEvicted int64
}
...
// captureScrollback copies the n lines about to be evicted from the top of the
// scroll region into the scrollback ring. It is only active for the main
// screen (orig == t.top, not the alternate screen) so full-screen interactive
// programs such as vim do not pollute the history. Called from scrollUp with
// t.mu held; must not lock.
func (t *State) captureScrollback(orig, n int) {
	if t.scrollbackMax <= 0 || n <= 0 || orig != t.top || t.mode&ModeAltScreen != 0 {
		return
	}
	for i := 0; i < n; i++ {
		src := t.lines[orig+i]
		// Trim trailing blank cells (default glyphs) to bound memory.
		end := len(src)
		for end > 0 && src[end-1].Char == 0 {
			end--
		}
		cp := make(line, end)
		copy(cp, src[:end])
	...
	t.scrollbackEvicted += int64(n)
	if len(t.scrollback) > t.scrollbackMax {
		excess := len(t.scrollback) - t.scrollbackMax
		// Compact down to the front so the backing array stays bounded.
		t.scrollback = t.scrollback[:copy(t.scrollback, t.scrollback[excess:])]
	}
}

// SetScrollbackMax sets the maximum number of scrolled-off lines retained.
// A value <= 0 disables (and clears) scrollback capture. Takes t.mu.
func (t *State) SetScrollbackMax(n int) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.scrollbackMax = n
	if n <= 0 {
		t.scrollback = nil
	} else if len(t.scrollback) > n {
		excess := len(t.scrollback) - n
		t.scrollback = t.scrollback[:copy(t.scrollback, t.scrollback[excess:])]
	}
}

// ScrollbackLen returns the number of lines currently in the scrollback ring.
// Caller must hold the lock (via Lock/Unlock) or otherwise serialize access.
func (t *State) ScrollbackLen() int {
	return len(t.scrollback)
}

// Scrollback returns a copy of the scrollback history, oldest line first.
// Each line is a copy of the evicted glyphs (trailing blanks trimmed).
// Caller must hold the lock (via Lock/Unlock) or otherwise serialize access.
...
// ClearScrollback discards all retained scrollback lines. Takes t.mu.
...
// into scrollback (continues past ring trimming). Caller must serialize access.
func (t *State) ScrollbackEvictedTotal() int64 {
	return t.scrollbackEvicted
}

// ScrollbackTail returns copies of the last n scrollback lines (fewer if the
// ring holds fewer), oldest of the returned set first. Caller must serialize
// access.
func (t *State) ScrollbackTail(n int) [][]Glyph {
	if n <= 0 {
		return nil
	}
	start := len(t.scrollback) - n
	if start < 0 {
		start = 0
	}
	tail := t.scrollback[start:]
	out := make([][]Glyph, len(tail))
	for i, ln := range tail {
		cp := make([]Glyph, len(ln))
		copy(cp, ln)
		out[i] = cp
...
func (t *State) setAttr(attr []int) {
	...
	for i := 0; i < len(attr); i++ {
		...
		case 21, 22: // normal intensity — clears both bold and faint
