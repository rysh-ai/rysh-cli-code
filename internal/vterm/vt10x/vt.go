...
type View interface {
	...
	// SetScrollbackMax sets the maximum number of scrolled-off main-screen
	// lines retained for scrollback. <= 0 disables and clears capture.
	SetScrollbackMax(n int)

	// ScrollbackLen returns the number of retained scrollback lines. Caller
	// must serialize access (Lock/Unlock).
	ScrollbackLen() int

	// Scrollback returns a copy of the scrollback history, oldest first.
	// Caller must serialize access (Lock/Unlock).
	Scrollback() [][]Glyph

	// ScrollbackEvictedTotal returns the monotonic count of lines ever captured
	// into scrollback (keeps growing past ring trimming). Serialize access.
	ScrollbackEvictedTotal() int64

	// ScrollbackTail returns copies of the last n scrollback lines, oldest of
	// the returned set first. Serialize access.
	ScrollbackTail(n int) [][]Glyph
